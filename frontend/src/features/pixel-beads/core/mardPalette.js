// MARD RGB data from maxcleme/beadcolors (MIT), contributor: Asher.
// Keep THIRD_PARTY_NOTICES.md with redistributed copies of this dataset.
const MARD_CSV = `
...
function naturalCode(left, right) {
  const parse = (value) => value.match(/^([A-Z]+)(\d+)$/)?.slice(1) || [value, '0']
  const [leftPrefix, leftNumber] = parse(left.code)
  const [rightPrefix, rightNumber] = parse(right.code)
  return leftPrefix.localeCompare(rightPrefix) || Number(leftNumber) - Number(rightNumber)
}

export const mardPalette = {
  id: 'mard-291-community',
  brand: 'MARD',
  name: 'MARD 291 色（社区校准）',
  source: 'https://github.com/maxcleme/beadcolors',
  colors: MARD_CSV.split('\n')
    .map((line) => {
      const [code, name, red, green, blue, contributor] = line.split(',')
      return {
